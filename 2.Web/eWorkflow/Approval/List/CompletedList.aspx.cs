...
public partial class Approval_List_CompletedList : DNSoft.eWF.FrameWork.Web.PageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!this.IsPostBack)
            {
                ViewState[VIEWSTATE_DETAIL_ITEMS_KEY] = new List<DTO_PROCESS_COMPLETED_LIST>();
                this.grdSearch.DataSource = (List<DTO_PROCESS_COMPLETED_LIST>)ViewState[VIEWSTATE_DETAIL_ITEMS_KEY];
    ...
    private void GridSource()
    {
        ...
        using (Bayer.eWF.BSL.Approval.Mgr.CommonMgr mgr = new Bayer.eWF.BSL.Approval.Mgr.CommonMgr())
        {
            ...
            grdSearch.DataSource = mgr.SelectApprovalCompletedList(condition.DOCUMENT_ID, condition.SUBJECT, chk, condition.FROM_DATE, condition.TO_DATE, Sessions.UserID);
        }       
    }
    #endregion
    ...
    protected void grdSearch_ItemDataBound(object sender, Telerik.Web.UI.GridItemEventArgs e)
    {
        ...
        {
            DTO_PROCESS_COMPLETED_LIST i = e.Item.DataItem as DTO_PROCESS_COMPLETED_LIST;
