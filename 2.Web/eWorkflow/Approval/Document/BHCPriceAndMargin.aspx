<%@ Page Title="" Language="C#" MasterPageFile="~/Master/eWorks_Document.master" AutoEventWireup="true" CodeFile="BHCPriceAndMargin.aspx.cs" Inherits="Approval_Document_BHCPriceAndMargin" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="Scripthead" runat="Server">
    <script type="text/javascript">
        ...
        function getTypeValue() {
            ...
            else if ($find('<%= radBtnSuEui.ClientID%>').get_checked())
                selectedValue = $find('<%= radBtnSuEui.ClientID %>').get_value();
...
<%--            else if ($find('<%= radBtnVirKon.ClientID %>').get_checked())
                selectedValue = $find('<%= radBtnVirKon.ClientID %>').get_value();--%>
           ...
           // alert($find('<%= radBtnVirKon.ClientID %>').get_value());
    ...
    function fn_OnHospitalRequesting(sender, args) {
        ...
        function fn_UpdateGridData(product) {
            ...
            for (var i = 0; i < dataItems.length; i++) {
                ...
                var HospitalMargin = dataItems[i].get_cell("HOSPITAL_MARGIN").innerText.replace(/,/gi, '').replace(/ /gi, '');
                var WholesalerMargin = dataItems[i].get_cell("WHOLESALER_MARGIN").innerText.replace(/,/gi, '').replace(/ /gi, '');
                ...
                conObj.HOSPITAL_MARGIN = HospitalMargin;
        ...
        function fn_OnGridKeyUp(sender) {
            ...
            if (dataItem) {
                ...
                // var ctlQty = dataItem.findElement('radNumSUPPLY_PRICE');
                var orginal_margin = parseFloat(dataItem.get_cell('MARGIN_AS_IS').innerText.replace(/,/gi, ''));
                

                //var supply = parseFloat(sender.value.replace(/,/gi, '')); //dataItem.findElement('BASE_PRICE');
                var supply = parseFloat(dataItem.findElement('radNumSUPPLY_PRICE').value.replace(/,/gi, ''));
                if (!supply) supply = 0;
                var base = parseFloat(dataItem.get_cell('BASE_PRICE').innerText.replace(/,/gi, '')); //dataItem.findElement('BASE_PRICE');
                if (!base) base = 0;
                // var w_margin = parseFloat(dataItem.get_cell('MARGIN_AS_IS').innerText.replace(/,/gi, '')); //dataItem.findElement('BASE_PRICE');
                ...
                // dataItem.get_cell('HOSPITAL_MARGIN').innerText = ((base-supply) / (base/100) ).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                dataItem.get_cell('HOSPITAL_MARGIN').innerText = ((base - supply) / (base / 100)).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                var h_margin = parseFloat(dataItem.get_cell('HOSPITAL_MARGIN').innerText.replace(/,/gi, ''));
                dataItem.get_cell('TOTAL_MARGIN').innerText = (w_margin + h_margin).toFixed(2).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
...
<asp:Content ID="Content2" ContentPlaceHolderID="HolderDocumentBody" runat="Server">
    <div class="doc_style">
                    ...
                    <td>
                        <div id="divSalesGroup" runat="server" style="margin: 0 0 0 0">
                            ...
                            <telerik:RadButton ID="radBtnDC" runat="server" Text="DC" Value="DC" GroupName="SalesGroup"
                                ButtonType="ToggleButton" ToggleType="Radio" OnClick="radBtnSalesGroup_Click" OnClientClicked="fn_RadioVisible"  Visible="false">
                            </telerik:RadButton>
                    ...
                    <td>
                        <div id="divProductFamily" runat="server" style="margin: 0 0 0 0">
                            ...
                            <telerik:RadButton ID="radBtnCVRM" runat="server" Text="CVRM" Value="CVRM" GroupName="HH"
                             ...
                             &nbsp;&nbsp;(2018-04-01 이후 요청한 문서부터 해당 자료가 정확히 표시되니 참고하시기 바랍니다.)
                        </div>
                    </td>
                </tr> 
                ...
                <tr id="typeNonAH" style="display: none">
                    ...
                    <td>
                        <telerik:RadButton ID="radBtnNormal" runat="server" Text="Normal" Value="Normal" GroupName="Type"
                            ...
                            <telerik:RadButton ID="radBtnSuEui" runat="server" Text="Su-Eui Contract" Value="SuEui Contract" GroupName="Type"
                                ButtonType="ToggleButton" ToggleType="Radio" OnClick="radBtnSalesGroup_Click">
                            </telerik:RadButton>
                            <telerik:RadButton ID="radBtnGateKeeper" runat="server" Text="GateKeeper" Value="GateKeeper" GroupName="Type"
                                ButtonType="ToggleButton" ToggleType="Radio" OnClick="radBtnSalesGroup_Click">
                            </telerik:RadButton>
        ...
        <telerik:RadGrid ID="radGrdProduct" runat="server" AutoGenerateColumns="false"
            ...
            <MasterTableView EditMode="Batch"  ClientDataKeyNames="IDX" TableLayout="Fixed" >
                ...
                <Columns>
                    ...
                    <telerik:GridTemplateColumn DataField="HOSPITAL_MARGIN" HeaderText="HOSPITAL<br/>MARGIN<br>(Given Figures)" UniqueName="HOSPITAL_MARGIN" HeaderStyle-Font-Size="10px"
                        HeaderStyle-Width="60px" Display="false" ColumnGroupName="Between">
                        <ItemStyle HorizontalAlign="Right" Wrap="false" />
                        <ItemTemplate>
                            <asp:Label ID="Label5" runat="server" Text='<%# String.Format("{0:#,##0.00}", Eval("HOSPITAL_MARGIN")) %>'></asp:Label>
                        </ItemTemplate>
                    ...
                    <telerik:GridTemplateColumn DataField="MARGIN_TO_BE" HeaderText="Margin%<br/>(To-Be)" UniqueName="MARGIN_TO_BE" HeaderStyle-Width="80px" HeaderStyle-Font-Size="10px">
                        <ItemStyle HorizontalAlign="Right" />
                        <ItemTemplate>
