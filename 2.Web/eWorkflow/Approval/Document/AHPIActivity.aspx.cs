...
public partial class Approval_Document_AHPIActivity : DNSoft.eWF.FrameWork.Web.WebBase.DocBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!this.IsPostBack)
            {
                List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL> initItem = new List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL>();

                ViewState[VIEWSTATE_KEY] = new List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL>();
                this.radGrdCostDetail.DataSource = (List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL>)ViewState[VIEWSTATE_KEY];
    ...
    private void InitPageInfo()
    {
        ...
        this.hddDocumentID.Value = "D0052";
    ...
    private void InitControls()
    {
        ...
        {
            List<DTO_COMMON_AH_PIACTIVITY> common = commonmgr.SelectCommonAHPIActivity(year);
        ...
        if (!this.hddProcessID.Value.IsNullOrEmptyEx())
        {
            ...
            {
                Tuple<DTO_DOC_AH_P_I_ACTIVITY, List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL>> documents = mgr.SelectAHPIActivity(this.hddProcessID.Value);

                DTO_DOC_AH_P_I_ACTIVITY doc = documents.Item1;
                this.radTxtTitle.Text = doc.TITLE;
                if (doc.BU.Equals(this.radBtnFAP_Swine.Value)) this.radBtnFAP_Swine.Checked = true;
                else if (doc.BU.Equals(this.radBtnFAP_SS.Value)) this.radBtnFAP_SS.Checked = true;
                else if (doc.BU.Equals(this.radBtnCAP.Value)) this.radBtnCAP.Checked = true;
    ...
    private void PageLoadInfo()
    {
        ...
        RadAjaxManager.GetCurrent(this).AjaxRequest += Approval_Document_AHPIActivity_AjaxRequest;
    ...
    void Approval_Document_AHPIActivity_AjaxRequest(object sender, AjaxRequestEventArgs e)
    {
    ...
    private void UpdateGridData()
    {
        ...
        List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL> items = (List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL>)serializer.Deserialize<List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL>>(this.hddGridItems.Value);
    ...
    protected void radGrdCostDetail_ItemCommand(object sender, Telerik.Web.UI.GridCommandEventArgs e)
    {
        if (e.CommandName.Equals("Remove"))
        {
            ...
            List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL> list = (List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL>)ViewState[VIEWSTATE_KEY];
    ...
    private string GetBU()
    {
        ...
        if (this.radBtnFAP_Swine.Checked) rtnValue = this.radBtnFAP_Swine.Value;
        else if (this.radBtnFAP_SS.Checked) rtnValue = this.radBtnFAP_SS.Value;
        else if (this.radBtnCAP.Checked) rtnValue = this.radBtnCAP.Value;
    ...
    {
        DTO_DOC_AH_P_I_ACTIVITY doc = new DTO_DOC_AH_P_I_ACTIVITY();
        ...
        List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL> details = (List<DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL>)ViewState[VIEWSTATE_KEY];
        bool existsPromotional = false;
        decimal? total = 0;
        foreach (DTO_DOC_AH_P_I_ACTIVITY_COST_DETAIL detail in details)
        {
        ...
        using (AHPIActivityMgr mgr = new AHPIActivityMgr())
        {
            return mgr.MergeAHPIActivity(doc, details);
        }
    }
