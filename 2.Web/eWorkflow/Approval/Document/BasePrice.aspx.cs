...
public partial class Approval_Document_BasePrice : DNSoft.eWF.FrameWork.Web.WebBase.DocBase
{
    private const string VIEWSTATE_KEY = "VIEWSTATE_KEY_BASE_PRICE";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                ViewState[VIEWSTATE_KEY] = new List<DTO_DOC_BASE_PRICE_PRODUCT>();
                this.RadGrdProduct.DataSource = (List<DTO_DOC_BASE_PRICE_PRODUCT>)ViewState[VIEWSTATE_KEY];
    ...
    {
        DTO_DOC_BASE_PRICE doc;

        if (!hddProcessID.Value.Equals(string.Empty))
        {
            using (BasePriceMgr mgr = new BasePriceMgr())
            {
                doc = mgr.SelectBasePrice(hddProcessID.Value);
                if (doc != null)
                {
                    ...
                    this.RadDatePeriod.SelectedDate = doc.CONTRACT_PERIOD;
                    // PRODUCT LIST
                    List<DTO_DOC_BASE_PRICE_PRODUCT> products = mgr.SelectBasePriceProduct(hddProcessID.Value);
    ...
    private bool ValidationCheck(ApprovalUtil.ApprovalStatus approvalStatus)
    {
        ...
        {
            if (RadrdoNew.Checked == false && RadrdoChange.Checked == false)
                message = "Category";
            ...
            if (!RadDatePeriod.SelectedDate.HasValue)
    ...
    {
        DTO_DOC_BASE_PRICE price = new DTO_DOC_BASE_PRICE();
        ...
        price.CONTRACT_PERIOD = RadDatePeriod.SelectedDate;

        price.IS_DISUSED = "N";
        price.CREATOR_ID = Sessions.UserID;
        price.CREATE_DATE = DateTime.Now;

        // List<DTO_DOC_BASE_PRICE_PRODUCT> products = (List<DTO_DOC_BASE_PRICE_PRODUCT>)ViewState[VIEWSTATE_KEY];
        JavaScriptSerializer serializer = new JavaScriptSerializer();
        List<DTO_DOC_BASE_PRICE_PRODUCT> products = (List<DTO_DOC_BASE_PRICE_PRODUCT>)serializer.Deserialize<List<DTO_DOC_BASE_PRICE_PRODUCT>>(this.hddGridItems.Value);
        foreach (DTO_DOC_BASE_PRICE_PRODUCT item in products)
        {
        ...
        using (BasePriceMgr mgr = new BasePriceMgr())
        {
            return mgr.MergeBasePrice(price, products);
        }
    }
    ...
    {
        JavaScriptSerializer serializer = new JavaScriptSerializer();
        List<DTO_DOC_BASE_PRICE_PRODUCT> items = (List<DTO_DOC_BASE_PRICE_PRODUCT>)serializer.Deserialize<List<DTO_DOC_BASE_PRICE_PRODUCT>>(this.hddGridItems.Value);
    ...
    protected void RadGrdProduct_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            if (e.CommandName.Equals("Remove"))
            {
                ...
                using (BasePriceDao Dao = new BasePriceDao())
                {
                    Dao.DeleteBasePriceProduct(this.hddProcessID.Value, productCode);
                }
                List<DTO_DOC_BASE_PRICE_PRODUCT> list = (List<DTO_DOC_BASE_PRICE_PRODUCT>)ViewState[VIEWSTATE_KEY];
