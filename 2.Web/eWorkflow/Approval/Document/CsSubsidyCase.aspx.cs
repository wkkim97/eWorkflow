...
public partial class Approval_Document_CsSubSidy : DNSoft.eWF.FrameWork.Web.WebBase.DocBase
{
    ...
    private void InitControls()
    {
        ...
        {
            using (CSSubsidycaseMgr mgr = new CSSubsidycaseMgr())
            {
                DTO_DOC_CS_SUBSIDY_CASE doc = mgr.SelectCSSubsidycase(this.hddProcessID.Value);
                ...
                List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT> products = mgr.SelectCSSubsidycaseProduct(this.hddProcessID.Value);
                ViewState[VIEWSTATE_BCS_AR_ADJUSTMENT_PRODUCT_KEY] = products;

                if (doc.TITLE_CODE == "Exception")
                {
                    foreach (DTO_DOC_CS_SUBSIDY_CASE_PRODUCT product in products)
                    {
    ...
    void Approval_Document_CSException_AjaxRequest(object sender, AjaxRequestEventArgs e)
    {
        ...
        {
            ViewState[VIEWSTATE_BCS_AR_ADJUSTMENT_PRODUCT_KEY] = new List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT>();
            this.radGrdSampleItemList.DataSource = (List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT>)ViewState[VIEWSTATE_BCS_AR_ADJUSTMENT_PRODUCT_KEY];
            this.radGrdSampleItemList.DataBind();
        }
    }
    ...
    private void UpdateGridData()
    {
        ...
        List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT> products = (List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT>)serializer.Deserialize<List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT>>(this.hddGridItems.Value);
        foreach (DTO_DOC_CS_SUBSIDY_CASE_PRODUCT product in products)
        {
    ...
    private bool ValidationCheck(ApprovalUtil.ApprovalStatus status)
    {
        ...
        if (status == ApprovalUtil.ApprovalStatus.Request)
        {
            ...
            if (radRdoTitle1.Checked == false && radRdoTitle2.Checked == false)
                message += "Title";
    ...
    protected void radGrdSampleItemList_ItemCommand(object sender, GridCommandEventArgs e)
    {
        try
        {
            if (e.CommandName.Equals("Remove"))
            {
                ...
                List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT> list = (List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT>)ViewState[VIEWSTATE_BCS_AR_ADJUSTMENT_PRODUCT_KEY];
    ...
    {
        DTO_DOC_CS_SUBSIDY_CASE doc = new DTO_DOC_CS_SUBSIDY_CASE();
        doc.PROCESS_ID = this.hddProcessID.Value;

        var strSelectedBU = GetSelectedBU();

        doc.SUBJECT = strSelectedBU + " / " +  "Subsidy case / " + this.radGrdTxtCustomer.Text;
        ...
        List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT> products ;
        if (GetTitleCode() == "Exception")
        {
            products = new List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT>();
            int idx_num = 1;
            foreach (AutoCompleteBoxEntry entry in this.radAutoProduct.Entries)
            {
                DTO_DOC_CS_SUBSIDY_CASE_PRODUCT product = new DTO_DOC_CS_SUBSIDY_CASE_PRODUCT();
        ...
        {
            JavaScriptSerializer serializer = new JavaScriptSerializer();
            products = (List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT>)serializer.Deserialize<List<DTO_DOC_CS_SUBSIDY_CASE_PRODUCT>>(this.hddGridItems.Value);
            foreach (DTO_DOC_CS_SUBSIDY_CASE_PRODUCT product in products)
            {
                product.PROCESS_ID = this.hddProcessID.Value;
                product.CREATOR_ID = Sessions.UserID;
        ...
        using (CSSubsidycaseMgr mgr = new CSSubsidycaseMgr())
        {
            return mgr.MergeCSSubsidycase(doc, products);
