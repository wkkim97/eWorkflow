...
public partial class Approval_Document_BHCPriceAndMargin : DNSoft.eWF.FrameWork.Web.WebBase.DocBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!this.IsPostBack)
            {
                ...
                //ViewState[VIEWSTATE_KEY] = new List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT>();
                //this.radGrdProduct.DataSource = (List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT>)ViewState[VIEWSTATE_KEY];
    ...
    private void InitControls()
    {
        ...
        {
            using (BHCPriceAndMargin mgr = new BHCPriceAndMargin())
            {

                Tuple<DTO_DOC_BHC_PRICE_AND_MARGIN
                , List<DTO_DOC_BHC_PRICE_AND_MARGIN_CUSTOMER>
                , List<DTO_DOC_BHC_PRICE_AND_MARGIN_HOSPITAL>
                , List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT>> doc = mgr.SelectBHCPriceAndMargin(this.hddProcessID.Value);
                ...
                if (doc.Item1.BU == "CH") {
                    ...
                    radBtnCH.Visible = true;

                    //radBtnCC.Checked = false;
                    radBtnCC.Visible = false;
                }
                else if (doc.Item1.BU == "CC")
                {
                    //radBtnCH.Checked = false;
                    radBtnCH.Visible = false;

                    //radBtnCC.Checked = true;
                    radBtnCC.Visible = true;
                }
                else if (doc.Item1.BU == "DC")
                {
                    //radBtnDC.Checked = false;
                    //radBtnDC.Visible = false;

                    radBtnDC.Checked = true;
                    radBtnDC.Visible = true;
                }
                ...
                List<DTO_DOC_BHC_PRICE_AND_MARGIN_CUSTOMER> customers = doc.Item2;
                foreach (DTO_DOC_BHC_PRICE_AND_MARGIN_CUSTOMER customer in customers)
                {
                ...
                List<DTO_DOC_BHC_PRICE_AND_MARGIN_HOSPITAL> hospitals = doc.Item3;
                foreach (DTO_DOC_BHC_PRICE_AND_MARGIN_HOSPITAL hospital in hospitals)
                {
                    AutoCompleteBoxEntry entry = new AutoCompleteBoxEntry();
                ...
                List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT> products = doc.Item4;
    ...
    private void PageLoadInfo()
    {
        ...
        RadAjaxManager.GetCurrent(this).AjaxRequest += Approval_Document_BHCPriceAndMargin_AjaxRequest;
    ...
    void Approval_Document_BHCPriceAndMargin_AjaxRequest(object sender, AjaxRequestEventArgs e)
    {
    ...
    private void UpdateGridData()
    {
        ...
        List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT> items
            = (List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT>)serializer.Deserialize<List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT>>(this.hddGridItems.Value);

        foreach (DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT product in items)
        {
    ...
    protected void radGrdProduct_ItemCommand(object sender, GridCommandEventArgs e)
    {
        if (e.CommandName.Equals("Remove"))
        {
            ...
            using (BHCPriceAndMargin mgr = new BHCPriceAndMargin())
            {
                mgr.DeleteBHCPriceAndMarginProduct(this.hddProcessID.Value, idx);
            }

            List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT> list = (List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT>)ViewState[VIEWSTATE_KEY];
    ...
    protected void radBtnDealNoSave_Click(object sender, EventArgs e)
    {
        ...
        using (Bayer.eWF.BSL.Approval.Mgr.BHCPriceAndMargin mgr = new Bayer.eWF.BSL.Approval.Mgr.BHCPriceAndMargin())
        {
    ...
    private void radRadioCheck()
    {
       ...
       // this.radBtnTender.Visible = false;
        ...
        this.radGrdProduct.MasterTableView.GetColumn("HOSPITAL_MARGIN").Display = false;
        ...
        if (salesGroup.Equals("CC") || salesGroup.Equals("CH") || salesGroup.Equals("DC"))
        {
        ...
        //    this.radBtnTender.Visible = false;
            ...
            //if (typeValue.Equals("Tender") || typeValue.Equals("Direct Contract"))
            //    (this.radGrdProduct.MasterTableView.GetColumn("MARGIN_AS_IS") as GridTemplateColumn).ReadOnly = true;
            //else if (typeValue.Equals("Normal"))
            //    (this.radGrdProduct.MasterTableView.GetColumn("MARGIN_AS_IS") as GridTemplateColumn).ReadOnly = true;
            //else if (typeValue.Equals("Gatekeeper"))
            //    (this.radGrdProduct.MasterTableView.GetColumn("MARGIN_AS_IS") as GridTemplateColumn).ReadOnly = true;
            (this.radGrdProduct.MasterTableView.GetColumn("MARGIN_AS_IS") as GridTemplateColumn).ReadOnly = true;
        }
        ...
        {
      //      this.radBtnTender.Visible = true;
            this.radGrdProduct.MasterTableView.GetColumn("MARGIN_AS_IS").Display = false;
            (this.radGrdProduct.MasterTableView.GetColumn("MARGIN_AS_IS") as GridTemplateColumn).ReadOnly = true;
            this.radGrdProduct.MasterTableView.GetColumn("MARGIN_TO_BE").Display = false;
            this.radGrdProduct.MasterTableView.GetColumn("DISCOUNT_AMOUNT").Display = false;
            ...
            if (typeValue.Equals("Tender") || typeValue.Equals("SuEui Contract"))
            {
                this.radGrdProduct.MasterTableView.GetColumn("SUPPLY_PRICE").Display = true;
                this.radGrdProduct.MasterTableView.GetColumn("HOSPITAL_MARGIN").Display = true;
                this.radGrdProduct.MasterTableView.GetColumn("MARGIN_AS_IS").Display = false;
    ...
    private bool ValidationCheck(ApprovalUtil.ApprovalStatus status)
    {
        ...
        if (status == ApprovalUtil.ApprovalStatus.Request)
        {
            ...
            //if (((List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT>)ViewState[VIEWSTATE_KEY]).Count < 1)
    ...
    {
        DTO_DOC_BHC_PRICE_AND_MARGIN doc = new DTO_DOC_BHC_PRICE_AND_MARGIN();
        ...
        List<DTO_DOC_BHC_PRICE_AND_MARGIN_CUSTOMER> customers = new List<DTO_DOC_BHC_PRICE_AND_MARGIN_CUSTOMER>();
        foreach (AutoCompleteBoxEntry entry in this.radAcomWholesaler.Entries)
        {
            DTO_DOC_BHC_PRICE_AND_MARGIN_CUSTOMER customer = new DTO_DOC_BHC_PRICE_AND_MARGIN_CUSTOMER();
        ...
        List<DTO_DOC_BHC_PRICE_AND_MARGIN_HOSPITAL> hospitals = new List<DTO_DOC_BHC_PRICE_AND_MARGIN_HOSPITAL>();
        foreach (AutoCompleteBoxEntry entry in this.radAcomHospital.Entries)
        {
            DTO_DOC_BHC_PRICE_AND_MARGIN_HOSPITAL hospital = new DTO_DOC_BHC_PRICE_AND_MARGIN_HOSPITAL();
        ...
        List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT> products = (List<DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT>)ViewState[VIEWSTATE_KEY];
        foreach (DTO_DOC_BHC_PRICE_AND_MARGIN_PRODUCT product in products)
        {
        ...
        using (BHCPriceAndMargin mgr = new BHCPriceAndMargin())
        {
            return mgr.MergeBHCPriceAndMargin(doc, customers, hospitals, products);
        }
