...
namespace Bayer.WCF.Service
{
    public class Impersonation : IDisposable
    {
        ...
        private static IntPtr ValidateParametersAndGetFirstLoginToken(string domain, string username, string password)
        {
            ...
            if (!RevertToSelf())
            {
                MailingService.WriteLog("RevertToSelf call to remove any prior impersonations failed" + System.Diagnostics.EventLogEntryType.Error);
            ...
            if (!result)
            {
                ...
                MailingService.WriteLog(string.Format("Could not impersonate the elevated user.  LogonUser: {2}\\{1} returned error code: {0}.", errorCode, username, domain) + System.Diagnostics.EventLogEntryType.Error);
